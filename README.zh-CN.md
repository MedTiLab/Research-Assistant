...
**UKB 分析智能体是科研助理内置的一项免费小工具**，用于辅助 UK Biobank 字段查询、变量提取与后续分析。如果你已有自己的 UK Biobank 数据，可以直接告诉 Agent 数据文件夹地址，配合变量提取，持续推进后续分析。
...
**MedHelp 同样支持使用你自己的数据。** 如果你已有 UK Biobank 或其他研究数据，可以直接告诉 Agent 数据文件夹地址和研究需求，让它结合变量检索与提取，逐步推进数据清理、统计分析和结果整理，并在持续交流中调整变量与分析方案。数据文件夹需能被运行 Agent 的电脑或计算节点访问。
...
> 我的 UK Biobank 数据位于 `/path/to/ukb-data`。请先查看文件结构，围绕我的研究问题梳理并提取所需变量，再和我一起持续推进后续分析。

将示例路径换成自己的实际地址，并确保运行 Agent 的电脑或计算节点能够访问该文件夹。
