...
await fs.copyFile(
  path.join(packagingSource, 'com.medtilab.researchassistant.kernel.plist'),
  path.join(launchAgentsDir, 'com.medtilab.researchassistant.kernel.plist'),
);
...
run('plutil', ['-lint', path.join(launchAgentsDir, 'com.medtilab.researchassistant.kernel.plist')]);
...
run('pkgbuild', [
  '--root', payloadRoot,
  '--scripts', scriptsDir,
  '--identifier', 'com.medtilab.researchassistant.kernel.pkg',
  '--version', version,
  '--install-location', '/',
