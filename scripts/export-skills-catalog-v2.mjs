...
function inferSource(skill) {
  ...
  if (explicitSource === 'vibelab' || explicitSource === 'medhelp' || explicitSource === 'medhelp') {
    return 'medhelp';
  }

  const normalizedSource = normalizeKey(skill.source);
  return normalizedSource === 'vibelab' || normalizedSource === 'medhelp' || normalizedSource === 'medhelp'
    ? 'medhelp'
    : 'imported';
}

function inferStatus(skill, source) {
  if (source === 'vibelab' || source === 'medhelp') {
    return 'verified';
  }
...
async function main() {
  ...
  for (const skillFile of skillFiles) {
    ...
    rawSkills.push({
      ...
      source: isPlatformNative ? 'medhelp' : 'imported',
      summary,
      fullDescription,
      tags: {
        stage: stageTags,
