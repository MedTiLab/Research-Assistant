...
MedHelp project routing: publication-quality does not automatically mean publication-stage. Save generated result figures, charts, plots, diagnostics, and analysis images under `Experiment/figures/` by default. Use `Publication/figures/` only when the user explicitly asks for finalized manuscript/submission figure panels or publication-stage figure legends.

MedHelp figure contract:

- Use Python plotting by default unless the user explicitly chooses another language or Python cannot complete the required workflow.
- Do not embed `Figure`, `Fig.`, `Figure 1`, or equivalent figure numbering/title text in the plotting canvas. Panel labels such as `A`, `B`, and `C` are allowed for multi-panel figures.
- Rewrite the standalone figure legend only after the final figure is rendered, using the actual panels, groups, encodings, units, sample sizes when relevant, uncertainty/error bars, statistical annotations, and abbreviations.
- Choose fonts, markers/icons, lines, and legend sizes for the final physical layout; keep them legible and balanced after downscaling, neither too small nor too large.
- Reopen or render-inspect each saved final figure once and correct clipping, overlap, inconsistent typography, disproportionate marker/icon sizes, legend mismatches, poor distinguishability, resolution problems, or disagreement with plotted values before completion.
...
- [ ] No `Figure`/`Fig.` number or figure title is embedded in the plotting canvas
- [ ] Standalone figure legend was rewritten from the final rendered output and actual analysis results
- [ ] Font, marker/icon, line, and legend sizes are balanced at final layout size
- [ ] Final saved figure was reopened or render-inspected once and any defects were corrected
