...
- Set `pipeline.startStage`:
  - Use `"literature"` (default) when user is starting from scratch or needs literature review first.
...
Stage order: `literature` < `ideation` < `experiment` < `publication` < `promotion`.
