...
For the database workspace under `/Users/gaoyuzhen/Web_database`, the skill layer is organized as **1 API access/router skill + 27 independent source skills**. The access layer is API-first through `medhelp-database-api-access`, with source-specific skills handling direct extraction workflows. The bundled client prefers `https://api.medtimehelp.com` and falls back to a healthy same-host `http://127.0.0.1:8787` only when the public endpoint is unavailable.
