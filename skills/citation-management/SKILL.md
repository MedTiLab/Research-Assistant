...
description: Comprehensive citation management for academic research. Search Google Scholar and PubMed for papers, extract accurate metadata, validate citations, and generate properly formatted BibTeX plus reference-manager-ready RIS/ENW files for Zotero, Mendeley, and EndNote. This skill should be used when you need to find papers, verify citation information, convert DOIs to BibTeX, export importable reference files, or ensure reference accuracy in scientific writing.
...
- Exporting references in formats importable by Zotero, Mendeley, EndNote, or other reference managers
...
Default final deliverables for any generated bibliography:
- `final_references.bib` for LaTeX, Overleaf, and reproducible manuscript workflows
- `references.ris` for direct import into Zotero, Mendeley, EndNote, Paperpile, and most reference managers

If the user specifically requests EndNote tagged format, also generate `references.enw`.

```bash
# Export a reference-manager-ready RIS file from the cleaned BibTeX bibliography
python scripts/export_reference_manager.py final_references.bib \
  --format ris \
  --output references.ris

# Optional EndNote tagged export
python scripts/export_reference_manager.py final_references.bib \
  --format enw \
  --output references.enw
```

Do not fabricate missing metadata to make the export look complete. If DOI, pages, volume, issue, or URL are absent after validation, leave those fields empty and note the gap in the validation report.
...
# 6. Export a reference-manager import file by default
python scripts/export_reference_manager.py final_references.bib \
  --format ris \
  --output references.ris

# 7. Review validation report and fix any remaining issues
cat validation.json
...
### export_reference_manager.py

Export a cleaned BibTeX bibliography to formats that reference managers can import.

**Features**:
- Converts BibTeX to RIS for Zotero, Mendeley, EndNote, Paperpile, and most citation managers
- Converts BibTeX to EndNote tagged `.enw` when EndNote-specific export is requested
- Preserves DOI, URL, volume, issue, pages, abstracts, keywords, ISBN, and ISSN when present
- Leaves missing metadata empty rather than inventing fields

**Usage**:
```bash
# Default recommended export
python scripts/export_reference_manager.py final_references.bib \
  --format ris \
  --output references.ris
...
4. **Always provide import files**:
   - Generate RIS automatically whenever producing a final bibliography
   - Keep the RIS export synchronized with the cleaned BibTeX file
   - Use ENW only when the user asks for EndNote tagged output
...
# Step 7: Export for Zotero, Mendeley, EndNote, or Paperpile import
python scripts/export_reference_manager.py final_references.bib \
  --format ris \
  --output references.ris
...
- `export_reference_manager.py`: BibTeX to RIS/ENW exporter for reference managers
...
5. **Reference manager export** to RIS/ENW for Zotero, Mendeley, EndNote, and Paperpile
6. **Quality assurance** through validation and reporting
7. **Integration** with scientific writing workflow
8. **Reproducibility** through documented search and extraction methods
