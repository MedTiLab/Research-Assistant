...
| `prepare_res`         | string | JSON with `reference_codebases` and `reference_paths` (from `medhelp-prepare-resources`) |
| `code_survey_res`     | string | Optional prior implementation or codebase review notes (stored as `model_survey` when available) |
