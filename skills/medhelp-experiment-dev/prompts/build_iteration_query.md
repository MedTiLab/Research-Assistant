...
| `core_code` | string | Path to `Experiment/core_code/` — absolute from `instance.Experiment.core_code` when created by MedHelp; use as-is or resolve if relative |
| `code_references` | string | Path to `Experiment/code_references/` — absolute from `instance.Experiment.code_references` when created by MedHelp; use as-is or resolve if relative |
