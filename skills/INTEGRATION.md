# Research Flow Integration (MedHelp)
...
- **Single entry**: One button or command, e.g. **"Start Research"**, in the MedHelp UI that kicks off the Research pipeline.
- **User input** (one of):
...
- **Request body** (example):
  ...
  - Optional: `category`, `max_iter_times`, `instance` (path to instance file; paths inside instance.json are typically **absolute** when created by MedHelp, or may be relative; consumers should use as-is when absolute).
...
- **Plan**: prepare (with ideas) → academic research/method review → medhelp-experiment-dev (plan + implement + judge + submit) → medhelp-experiment-analysis (analyse + refine) → medhelp-paper-writing (optional). Use `run_infer.py` with `mode=plan` and `ideas=task1` or equivalent.
- **Idea**: prepare → idea-generation → idea-eval (quality gate) → academic research/method review → medhelp-experiment-dev (plan + implement + judge + submit) → medhelp-experiment-analysis (analyse + refine) → medhelp-paper-writing (optional). Use `run_infer_idea_ours.py` or equivalent.

Skills in `skills/` (MedHelp repo root) document inputs, outputs, and Python references for each step; the backend can call the existing Python entry points or re-use the same prompt/agent logic in another runtime. When a project is created, MedHelp symlinks these into the project's `.claude/skills/` so Claude can discover them.
