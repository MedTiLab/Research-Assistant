...
import { resolveAppRuntimeDir } from './storagePaths.js';
...
function getDefaultRuntimeDir() {
    return resolveAppRuntimeDir();
}

function getRuntimePortsPath() {
    if (process.env.MEDHELP_RUNTIME_FILE) {
        return process.env.MEDHELP_RUNTIME_FILE;
    }
    ...
    const runtimeDir = process.env.MEDHELP_RUNTIME_DIR
