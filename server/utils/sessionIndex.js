...
function normalizeStageTagKeys(stageTagKeys = []) {
  const normalized = Array.from(new Set(
    (Array.isArray(stageTagKeys) ? stageTagKeys : [])
      ...
      .map((value) => (value === 'survey' || value === 'research' ? 'literature' : value))
      .filter((value) => ['literature', 'ideation', 'experiment', 'publication', 'promotion'].includes(value))
  ));
...
export function recordIndexedSession({
  ...
  sessionDb.upsertSessionPlaceholder(
    ...
    lastActivity || new Date().toISOString(),
    {
      ...
      displayNameSource: displayName ? 'user' : 'placeholder',
    },
  );
