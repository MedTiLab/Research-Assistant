...
const SYSTEM_CLAUDE_WORKDIR = path.join(os.tmpdir(), 'medhelp-claude-system');
...
const EXTRACTION_PROVIDERS = ['claude', 'codex', 'local'];
...
function isModelValidForProvider(provider, model) {
  ...
  const options = provider === 'codex' ? CODEX_MODELS.OPTIONS : CLAUDE_MODELS.OPTIONS;
  return options.some((entry) => entry.value === model);
}
...
function getDefaultModelForProvider(provider) {
  ...
  return CLAUDE_MODELS.DEFAULT || 'opus';
}
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (code) => {
      ...
      try {
        ...
        finish(Boolean(status?.loggedIn));
      } catch {
        finish(false);
      }
    });
...
async function hasClaudeCredentials() {
  if (process.env.ANTHROPIC_AUTH_TOKEN || process.env.ANTHROPIC_API_KEY) {
    return true;
  }
  return (await fileExists(path.join(os.homedir(), '.claude', '.credentials.json')))
    || (await hasClaudeCliAuth());
}
...
class HeadlessTextCollector {
  ...
  send(payload) {
    ...
    if (payload.type === 'claude-response') {
      const deltaText = payload.data?.delta?.text;
      if (typeof deltaText === 'string' && deltaText) {
        this.collectText(deltaText);
      }
      this.collectContentParts(payload.data?.message?.content);
      this.collectContentParts(payload.data?.content);
    }
...
async function callClaudeHeadless(messages, extractionConfig, signal) {
  throwIfAborted(signal);
  if (!await hasClaudeCredentials()) {
    return null;
  }

  await fs.mkdir(SYSTEM_CLAUDE_WORKDIR, { recursive: true });
  const collector = new HeadlessTextCollector();
  await queryClaudeSDK(buildCombinedPrompt(messages), {
    cwd: SYSTEM_CLAUDE_WORKDIR,
...
export async function requestStructuredJson({ userId, reference, sourceKey, extractionConfig, overrideMessages, signal }) {
  ...
  return callClaudeHeadless(messages, extractionConfig, signal);
}
