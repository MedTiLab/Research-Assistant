...
export function buildAllowedDataFoldersEnv(folders) {
  if (!folders.length) return {};
  const env = { MEDHELP_ALLOWED_DATA_FOLDERS: JSON.stringify(folders) };
  for (const folder of folders) {
    for (const appRoot of [folder, path.join(folder, 'Web_database')]) {
      if (!env.MEDHELP_LOCAL_DATABASE_APP_ROOT && fs.existsSync(path.join(appRoot, 'server.js'))) {
        env.MEDHELP_LOCAL_DATABASE_APP_ROOT = appRoot;
      }
      const rawRoot = path.join(appRoot, 'database', 'data', 'sources');
      if (!env.MEDHELP_LOCAL_DATABASE_RAW_ROOT && fs.existsSync(rawRoot)) {
        env.MEDHELP_LOCAL_DATABASE_RAW_ROOT = rawRoot;
      }
    }
  }
  return env;
...
export function getAllowedDataFoldersAgentEnv(options = {}) {
  return buildAllowedDataFoldersEnv(getConfiguredAllowedDataFolders(options));
}

export function withAllowedDataFoldersAgentEnv(baseEnv = {}, options = {}) {
  const env = { ...baseEnv };
  // Also remove stale values after a folder is removed or a drive is unmounted.
  for (const key of DATA_ENV_KEYS) delete env[key];
  return { ...env, ...getAllowedDataFoldersAgentEnv(options) };
}
