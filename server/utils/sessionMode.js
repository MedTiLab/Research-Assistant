const EXPLICIT_SESSION_MODES = new Set(['research', 'workspace_qa', 'consultation']);
...
export function normalizeSessionMode(value) {
  ...
  return EXPLICIT_SESSION_MODES.has(normalized) ? normalized : 'research';
}
...
export function extractSessionModeFromText(value) {
  ...
  if (/\[Context:\s*session-mode=consultation\]/i.test(text)) {
    return 'consultation';
  }
