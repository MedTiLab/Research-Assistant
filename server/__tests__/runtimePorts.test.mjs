...
afterEach(async () => {
  ...
  delete process.env.MEDHELP_RUNTIME_DIR;
  delete process.env.MEDHELP_RUNTIME_FILE;
});
...
describe('runtimePorts', () => {
  ...
  it('reads runtime ports from an override directory and ignores stale process entries', async () => {
    const runtimeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'medhelp-runtime-'));
  ...
  it('prefers medhelp runtime overrides over legacy dr-claw overrides', async () => {
    const legacyRuntimeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'dr-claw-runtime-'));
    const currentRuntimeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'medhelp-runtime-'));
    process.env.DR_CLAW_RUNTIME_DIR = legacyRuntimeDir;
    process.env.MEDHELP_RUNTIME_DIR = currentRuntimeDir;
    cleanupTasks.push(() => fs.rm(legacyRuntimeDir, { recursive: true, force: true }));
    cleanupTasks.push(() => fs.rm(currentRuntimeDir, { recursive: true, force: true }));

    setRuntimePortSync('backend', 4555);

    expect(getBackendPortSync(3001)).toBe(4555);
  });
});
