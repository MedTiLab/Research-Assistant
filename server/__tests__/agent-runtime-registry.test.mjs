...
const runtimeMocks = vi.hoisted(() => ({
  ...
  abortCodex: vi.fn(),
  compactCodex: vi.fn(),
  getActiveClaude: vi.fn(),
  getActiveCodex: vi.fn(),
  getClaudeStartTime: vi.fn(),
  getCodexStartTime: vi.fn(),
  isClaudeActive: vi.fn(),
  isCodexActive: vi.fn(),
  isCodexPlaceholder: vi.fn(),
  queryClaude: vi.fn(),
  queryCodex: vi.fn(),
  resolveClaudeToolApproval: vi.fn(),
  shutdownCodex: vi.fn(),
  steerClaude: vi.fn(),
  steerCodex: vi.fn(),
}));
...
vi.mock('../claude-sdk.js', () => ({
  abortClaudeSDKSession: runtimeMocks.abortClaude,
  getActiveClaudeSDKSessions: runtimeMocks.getActiveClaude,
  getClaudeSDKSessionStartTime: runtimeMocks.getClaudeStartTime,
  isClaudeSDKSessionActive: runtimeMocks.isClaudeActive,
  queryClaudeSDK: runtimeMocks.queryClaude,
  resolveToolApproval: runtimeMocks.resolveClaudeToolApproval,
  steerClaudeSDKSession: runtimeMocks.steerClaude,
}));
...
describe('Claude runtime adapter', () => {
  it('delegates lifecycle calls to the existing Claude SDK functions', async () => {
    const writer = { send: vi.fn() };
    const options = { sessionId: 'claude-session' };
    runtimeMocks.queryClaude.mockResolvedValue('query-result');
    runtimeMocks.steerClaude.mockResolvedValue({ success: true });
    runtimeMocks.abortClaude.mockResolvedValue(true);
    runtimeMocks.isClaudeActive.mockReturnValue(true);
    runtimeMocks.getActiveClaude.mockReturnValue(['claude-session']);
    runtimeMocks.getClaudeStartTime.mockReturnValue(123);

    await expect(claudeRuntime.start('start', options, writer)).resolves.toBe('query-result');
    await expect(claudeRuntime.resume('resume', options, writer)).resolves.toBe('query-result');
    await expect(claudeRuntime.steer('claude-session', 'adjust')).resolves.toEqual({ success: true });
    await expect(claudeRuntime.abort('claude-session')).resolves.toBe(true);
    expect(claudeRuntime.isActive('claude-session')).toBe(true);
    expect(claudeRuntime.getActiveSessions()).toEqual(['claude-session']);
    expect(claudeRuntime.getStartTime('claude-session')).toBe(123);

    expect(runtimeMocks.queryClaude).toHaveBeenNthCalledWith(1, 'start', options, writer);
    expect(runtimeMocks.queryClaude).toHaveBeenNthCalledWith(2, 'resume', options, writer);
    expect(runtimeMocks.steerClaude).toHaveBeenCalledWith('claude-session', 'adjust');
    expect(runtimeMocks.abortClaude).toHaveBeenCalledWith('claude-session');
  });

  it('keeps Claude capabilities and native permission handling provider-specific', () => {
    ...
    expect(claudeRuntime.native.resolveToolApproval).toBe(runtimeMocks.resolveClaudeToolApproval);
  });
});
