...
import { codexForkPoints, conversationForkPoints } from '../../utils/sessionForking.js';
...
const claudeSessionStore = new ClaudeSessionStore({
  list: async (projectIdentity, options = {}) => getSessions(
    projectIdentity.projectKey,
    options.limit ?? 0,
    options.offset ?? 0,
    ownerKeyToUserId(projectIdentity.ownerKey),
  ),
  read: async (identity, options = {}) => normalizeTranscriptResult(await getSessionMessages(
    identity.projectKey,
    identity.sessionId,
    options.limit ?? null,
    options.offset ?? 0,
  ...
  forkPoints: async (identity, options = {}) => {
    ...
    return conversationForkPoints(messages, {
      id: (entry) => entry?.uuid,
      role: (entry) => entry?.message?.role || entry?.role || entry?.type,
      text: (entry) => entry?.message?.content ?? entry?.content,
    });
  },
  fork: async (identity, input, options = {}) => {
    // Keep provider runtimes out of the shared session-listing path. Loading the
    // Claude SDK here eagerly can interfere with legacy transcript discovery.
    const { forkClaudeSession } = await import('../../claude-sdk.js');
    return forkClaudeSession(identity.sessionId, {
      projectPath: options.projectPath,
      pointId: input.pointId,
      title: input.title,
    });
  },
  rename: async (identity, title, options = {}) => renameSession(
    identity.projectKey,
    identity.sessionId,
    title,
    options.provider || 'claude',
  ...
  delete: async (identity, options = {}) => deleteSession(
    identity.projectKey,
    identity.sessionId,
    options.provider || 'claude',
    ownerKeyToUserId(identity.ownerKey),
  ),
  getUsage: async (identity, options = {}) => {
    const transcript = normalizeTranscriptResult(await getSessionMessages(
      identity.projectKey,
      identity.sessionId,
      null,
      0,
  ...
  reconcile: async (identity) => reconcileClaudeSessionIndex(identity.projectKey, identity.sessionId),
  watchRoots: () => [path.join(os.homedir(), '.claude', 'projects')],
});
