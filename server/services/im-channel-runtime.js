...
import { addProjectManually, createConversationWorkspace, ensureDefaultConversationProject } from '../projects.js';
...
const IM_DEFAULT_CONVERSATION_NAME = '对话和任务';
const IM_PROJECT_REQUIRED_REPLY = [
  '暂时无法打开「对话和任务」工作区，请先选择一个项目再开始聊天。',
  '发送 /projects 查看项目，/use 1 选择项目，/create 项目名 新建项目，/help 查看全部命令。',
].join('\n');
const IM_HELP_TEXT = [
  'MedHelp IM 命令：',
  '未选择研究项目时，消息会进入「对话和任务」，可以直接发消息。',
  '/projects - 查看可用项目',
  '/find 关键词 - 搜索项目',
  '/use 编号或项目名 - 选择当前对话的项目',
  '/project - 查看当前项目',
  '/create 项目名 - 在工作区中新建研究项目并选中',
  '/new - 新开一个智能体会话；在「对话和任务」中会新开对话文件夹',
  '/help - 查看这段帮助',
].join('\n');
...
function normalizeProjectRow(row) {
  if (!row) return null;
  const isDefaultWorkspace = row.metadata?.isDefaultWorkspace === true;
  return {
    id: row.id,
    displayName: isDefaultWorkspace
      ? IM_DEFAULT_CONVERSATION_NAME
      : (trimString(row.display_name) || row.id),
    path: trimString(row.path),
    lastAccessed: row.last_accessed || null,
    isStarred: row.is_starred === 1 || row.is_starred === true,
    isDefaultWorkspace,
    isConversationWorkspace: row.metadata?.isConversationWorkspace === true,
    metadata: row.metadata || null,
  };
}

async function ensureImDefaultConversationProject(userId) {
  const user = typeof userDb.getWorkspaceRootUser === 'function'
    ? userDb.getWorkspaceRootUser(userId)
    : null;
  const configuredRoot = trimString(user?.workspace_root)
    || trimString(process.env.WORKSPACES_ROOT)
    || null;
  const projectId = await ensureDefaultConversationProject(projectDb, userId, configuredRoot);
  return normalizeProjectRow(projectDb.getProjectById(projectId));
}

async function listUserProjects(userId) {
  await ensureImDefaultConversationProject(userId);
  return projectDb.getAllProjects(userId)
    ...
    .sort((left, right) => {
      if (left.isDefaultWorkspace !== right.isDefaultWorkspace) {
        return left.isDefaultWorkspace ? -1 : 1;
      }
...
function formatProjectList(projects, emptyText = '当前账号还没有项目。') {
  ...
  const lines = projects.slice(0, 12).map((project, index) => (
    `${index + 1}. ${project.displayName}${project.isDefaultWorkspace ? '（可直接发消息）' : ''}\n   ${project.path}`
  ));
  if (projects.length > 12) {
    lines.push(`还有 ${projects.length - 12} 个项目。发送 /find 关键词 缩小范围。`);
  }
  lines.push('发送 /use 编号或项目名 选择研究项目；不选则默认进入「对话和任务」。');
  return lines.join('\n');
}

async function resolveProjectReference(runtime, chatId, rawRef) {
  ...
  const allProjects = await listUserProjects(runtime.userId);
...
async function handleImControlCommand(runtime, chatId, text) {
  ...
  if (lower === '/projects' || lower === 'projects' || lower === '项目') {
    const projects = await listUserProjects(runtime.userId);
  ...
  if (lower.startsWith('/find ')) {
    ...
    const projects = (await listUserProjects(runtime.userId)).filter((project) => (
  ...
  if (lower === '/project' || lower === 'project') {
    const currentProject = runtime.selectedProjectsByChat.get(chatId)
      || await ensureImDefaultConversationProject(runtime.userId);
    if (!currentProject?.path) {
      return IM_PROJECT_REQUIRED_REPLY;
    }
    if (!runtime.selectedProjectsByChat.has(chatId)) {
      runtime.selectedProjectsByChat.set(chatId, currentProject);
    }
    return `当前项目：${currentProject.displayName}\n${currentProject.path}`;
  }

  if (lower.startsWith('/use ')) {
    const project = await resolveProjectReference(runtime, chatId, trimmed.slice('/use '.length));
    if (!project) {
  ...
  if (selectPrefix) {
    const project = await resolveProjectReference(runtime, chatId, trimmed.slice(selectPrefix.length));
    if (!project) {
      return '没有找到这个项目。发送 /projects 查看列表，或 /find 关键词 搜索。';
    }
    runtime.selectedProjectsByChat.set(chatId, project);
  ...
  if (lower === '/clear-project') {
    const general = await ensureImDefaultConversationProject(runtime.userId);
    runtime.sessionsByChat.delete(chatId);
    if (!general?.path) {
      runtime.selectedProjectsByChat.delete(chatId);
      return IM_PROJECT_REQUIRED_REPLY;
    }
    runtime.selectedProjectsByChat.set(chatId, general);
    return `已切换到「对话和任务」。\n${general.path}\n现在可以直接发消息；发送 /use 可改到研究项目。`;
  }
...
async function allocateConversationWorkspace(runtime, chatId, general) {
  const project = await createConversationWorkspace(general.path, runtime.userId);
  const selected = normalizeProjectRow({
    id: project.name,
  ...
  if (projectsUpdatedBroadcaster) {
    await projectsUpdatedBroadcaster(runtime.userId, {
      changeType: 'im-conversation-created',
      projectName: selected.id,
      projectPath: selected.path,
    }).catch((error) => {
      console.warn(`[IM] Failed to broadcast conversation update for user ${runtime.userId}:`, error.message);
    });
  }
  return selected;
}

export async function dispatchImText(runtime, { chatId, text, sendReply, sendTyping = null }) {
  ...
  let selectedProject = runtime.selectedProjectsByChat.get(normalizedChatId);
  if (!selectedProject?.path) {
    selectedProject = await ensureImDefaultConversationProject(runtime.userId);
    if (!selectedProject?.path) {
      await reply(IM_PROJECT_REQUIRED_REPLY).catch(() => null);
      return;
    }
    runtime.selectedProjectsByChat.set(normalizedChatId, selectedProject);
  }

  const mapped = resolveImSession(runtime, normalizedChatId, normalizedText);
  if (mapped.command === 'new' && (selectedProject.isDefaultWorkspace || selectedProject.isConversationWorkspace)) {
    selectedProject = await ensureImDefaultConversationProject(runtime.userId);
    if (selectedProject?.path) {
      runtime.selectedProjectsByChat.set(normalizedChatId, selectedProject);
    }
  ...
  if (selectedProject.isDefaultWorkspace && mapped.message) {
    selectedProject = await allocateConversationWorkspace(runtime, normalizedChatId, selectedProject);
  }
