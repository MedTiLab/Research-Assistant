...
import { buildLocalMemorySnapshot, parseLinuxFreeMemoryLine } from '../utils/computeMonitoring.js';
...
router.get('/local/monitor', async (_req, res) => {
  ...
  try {
    ...
    let linuxFreeMemoryLine = '';
    let memoryPressureOutput = '';
    let vmStatOutput = '';
    ...
    if (platform === 'darwin') {
      ...
      memoryPressureOutput = await run('memory_pressure -Q 2>/dev/null');
      if (!memoryPressureOutput) {
        vmStatOutput = await run('vm_stat 2>/dev/null');
      }
    } else if (platform === 'linux') {
      linuxFreeMemoryLine = await run('free -m 2>/dev/null | grep Mem');
    }

    const memorySnapshot = buildLocalMemorySnapshot({
      platform,
      totalMB: totalMemMB,
      osFreeMB: freeMemMB,
      linuxFreeMemoryLine,
