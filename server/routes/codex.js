...
import { getCodexSessions, getCodexSessionMessages } from '../projects.js';
...
import { compactCodexSession } from '../openai-codex.js';
...
router.post('/sessions/:sessionId/compact', async (req, res) => {
  try {
    const { env } = await buildManagedAgentSessionContextForRequest(req);
    const result = await compactCodexSession(req.params.sessionId, {
      userId: req.user?.id,
      env,
    });
    res.json({
      ...result,
      message: 'Codex context compaction started.',
    });
  } catch (error) {
    const statusCode = Number.isInteger(error?.statusCode) ? error.statusCode : 500;
    console.error(`Error compacting Codex session ${req.params.sessionId}:`, error);
    res.status(statusCode).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to compact Codex context.',
    });
  }
});
...
router.delete('/sessions/:sessionId', async (req, res) => {
  try {
    const { sessionId } = req.params;
    const { mode, projectName: requestedProjectName } = req.query;
    const deleteMode = typeof mode === 'string' ? mode : 'trash';
    const requestedOwnerKey = req.user?.id === null || req.user?.id === undefined
      ? null
      : String(req.user.id);
    ...
    const indexedSession = sessionDb.getSessionById(sessionId, {
      ...
      ...(typeof requestedProjectName === 'string' && requestedProjectName.trim()
        ? { projectName: requestedProjectName.trim() }
        : {}),
      ...(requestedOwnerKey ? { ownerKey: requestedOwnerKey } : {}),
    });
    const projectName = typeof requestedProjectName === 'string' && requestedProjectName.trim()
      ? requestedProjectName.trim()
      : indexedSession?.project_name;

    if (!projectName) {
      return res.status(404).json({
        success: false,
        error: 'Codex session is not indexed to a project. Refresh the project list and try again.',
      });
    }

    const identity = createAgentSessionIdentity({
      ownerKey: indexedSession?.ownerKey || requestedOwnerKey || 'local',
      projectKey: projectName,
      runtimeId: 'codex',
      sessionId,
    });
    const store = runtimeSessionStoreRegistry.require('codex');

    if (deleteMode === 'physical') {
      await store.delete(identity);
      return res.json({ success: true, mode: 'physical', projectName });
    }

    await store.trash(identity);
    res.json({ success: true, mode: 'trash', projectName });
  } catch (error) {
...
router.get('/mcp/cli/list', async (req, res) => {
  try {
    ...
    const codexCommand = await resolveCodexCliExecutable();
    if (!codexCommand) {
      return respond(503, { error: 'Codex CLI not installed' });
    }
    const proc = spawn(codexCommand, ['mcp', 'list'], { stdio: ['pipe', 'pipe', 'pipe'] });
...
router.post('/mcp/cli/add', async (req, res) => {
  try {
    ...
    const codexCommand = await resolveCodexCliExecutable();
    if (!codexCommand) {
      return res.status(503).json({ error: 'Codex CLI not installed' });
    }
    const proc = spawn(codexCommand, cliArgs, { stdio: ['pipe', 'pipe', 'pipe'] });
...
router.delete('/mcp/cli/remove/:name', async (req, res) => {
  try {
    ...
    const proc = spawn(codexCommand, ['mcp', 'remove', name], { stdio: ['pipe', 'pipe', 'pipe'] });
...
router.get('/mcp/cli/get/:name', async (req, res) => {
  try {
    ...
    const proc = spawn(codexCommand, ['mcp', 'get', name], { stdio: ['pipe', 'pipe', 'pipe'] });
