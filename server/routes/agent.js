...
import { getUserWorkspacesRoot, validateUserWorkspacePath } from './projects.js';
import {
  executeAgentTurn,
  mergeRuntimeUsage,
} from '../agent-runtime/index.js';
import { normalizeRuntimeObservations } from '../agent-runtime/observations/index.js';
import { buildManagedAgentSessionContextForRequest } from '../utils/agentSessionEnv.js';
import { authorizeCloudCapability, getAgentRuntimeEnvState } from '../utils/cloudAgentRuntimeEnv.js';
import { authorize as authorizeEntitlement } from '../utils/entitlements.js';
...
async function buildAgentRuntimeContextForRequest(req) {
  const context = await buildManagedAgentSessionContextForRequest(req);
  console.log('[database-api] REST agent runtime env state:', getAgentRuntimeEnvState(context.env));
  return context;
}
...
async function resolveAgentWorkspacePath(requestedPath, userId) {
  ...
  if (!validation.valid) {
    const error = new Error(validation.error || 'Project path must be inside your workspace');
    error.statusCode = 403;
    throw error;
  }

  return validation.resolvedPath || path.resolve(requestedPath.trim());
}

function isStrictPathChild(parentPath, candidatePath) {
  const relative = path.relative(path.resolve(parentPath), path.resolve(candidatePath));
  return Boolean(relative) && relative !== '..' && !relative.startsWith(`..${path.sep}`) && !path.isAbsolute(relative);
}

/**
 * Clean up a temporary external project and any safely-known provider state.
 * Codex persistence is provider-managed and is intentionally not guessed here.
 */
export async function cleanupProject({
  projectPath,
  provider,
  sessionId = null,
  allowedExternalRoot = null,
}) {
  try {
    ...
    // Only clean up projects in known external-projects directories.
    const cleanupAllowed = allowedRoots.some((root) => isStrictPathChild(root, resolvedProjectPath));
    if (!cleanupAllowed) {
      console.warn('⚠️ Refusing to clean up non-external project:', projectPath);
      return;
    }

    console.log('🧹 Cleaning up project:', resolvedProjectPath);
    await fs.rm(resolvedProjectPath, { recursive: true, force: true });
    console.log('✅ Project cleaned up');

    // Preserve the legacy Claude cleanup path. Codex session persistence is
    // managed by Codex indexing/app-server code and has no safe cleanup hook yet.
    if (provider === 'claude' && sessionId) {
      try {
        const sessionRoot = path.join(os.homedir(), '.claude', 'sessions');
        const sessionPath = path.resolve(sessionRoot, String(sessionId));
        if (!isStrictPathChild(sessionRoot, sessionPath)) {
          console.warn('⚠️ Refusing to clean up invalid Claude session path:', sessionId);
          return;
        }
    ...
    } else if (provider === 'codex' && sessionId) {
      console.log('ℹ️ Skipping Codex provider session cleanup; no safe native cleanup hook is registered.');
    }
  } catch (error) {
...
export class ResponseCollector {
  ...
  /**
   * Get the legacy provider-native assistant messages for API compatibility.
   */
  getAssistantMessages() {
    ...
    for (const msg of this.messages) {
      ...
      if (
        ...
        && data.data?.type === 'item'
        && data.data?.itemType === 'agent_message'
        && data.data?.message?.role === 'assistant'
      ) {
        assistantMessages.push(data.data.message);
      }
    }
  ...
  /**
   * Get provider-neutral assistant messages through Runtime Observation v1.
   */
  getNormalizedAssistantMessages() {
    const assistantMessages = [];

    for (const msg of this.messages) {
      const observations = normalizeRuntimeObservations(msg, { provider: this.provider });
      for (const observation of observations) {
        if (observation.type !== 'assistant_text') {
          continue;
        }
        assistantMessages.push(observation.message);
      }
    }
  ...
  getTotalTokens() {
    ...
    for (const msg of this.messages) {
      const observations = normalizeRuntimeObservations(msg, { provider });
      for (const observation of observations) {
        provider ||= observation.provider;
        if (observation.type === 'usage_updated') {
          usages.push(observation.usage);
        }
      }
    }

    return mergeRuntimeUsage(provider, usages);
  }
}
 ...
 *                        Codex models: dynamically resolved from the configured Codex provider catalog.
 *                                      The app follows the provider defined in ~/.codex/config.toml
 *                                      and falls back to bundled GPT-5 entries such as 'gpt-5.4'.
 *
 ...
 *     messages: [...],        // Legacy provider-native assistant messages
 *     normalizedMessages: [   // Provider-neutral assistant messages
 ...
 *       reasoningTokens: null | 0,
 *       context: null | { used: 200, total: 256000, estimated: false },
 *       totalTokens: 200,
 *       rawSemantics: "claude" | "codex"
 *     },
...
router.post('/', validateExternalApiKey, async (req, res) => {
  ...
  const capability = provider === 'codex' ? 'agent.codex' : 'agent.claude';
  let access;
  try {
    access = req.localKernelSession
      ? await authorizeCloudCapability(req.localKernelSession, capability)
      : authorizeEntitlement(req.user?.id, capability);
  } catch (error) {
    console.error('[agent] Entitlement check failed:', error.message);
  ...
  if (!access.allowed) {
    return res.status(access.status || 403).json({
      error: access.code === 'CAPABILITY_DENIED'
        ? `${provider === 'codex' ? 'Codex' : 'Claude'} is available on the Pro plan. Free accounts can use Pi Agent.`
        : access.reason,
      code: access.code,
      capability,
      requiredPlan: 'pro',
  ...
  let externalProjectCleanupRoot = null;
  let writer = null;
  ...
  try {
    ...
    if (githubUrl) {
      ...
      let targetPath;
      if (projectPath) {
        targetPath = await resolveAgentWorkspacePath(projectPath, req.user?.id);
      } else {
        // Generate a unique path for cloning inside the caller's workspace.
        const userWorkspaceRoot = await getUserWorkspacesRoot(req.user?.id);
        externalProjectCleanupRoot = path.join(userWorkspaceRoot, 'external-projects');
        const repoHash = crypto.createHash('md5').update(githubUrl + Date.now()).digest('hex');
        targetPath = path.join(externalProjectCleanupRoot, repoHash);
      }

      finalProjectPath = await cloneGitHubRepo(githubUrl.trim(), tokenToUse, targetPath);
    } else {
      // Use existing project path
      finalProjectPath = await resolveAgentWorkspacePath(projectPath, req.user?.id);
    ...
    if (stream) {
      ...
      writer = new ResponseCollector({ provider });
    ...
    const { env: runtimeEnv, userPreferenceContext } = await buildAgentRuntimeContextForRequest(req);

    console.log(`🤖 Starting ${provider === 'codex' ? 'Codex' : 'Claude'} SDK session`);

    await executeAgentTurn({
      identity: {
        ownerKey: String(req.user?.id ?? 'local'),
        projectKey: project?.name || finalProjectPath,
        runtimeId: provider,
        sessionId: `agent-route-${crypto.randomUUID()}`,
      },
      runtimeId: provider,
      ...
      options: {
        ...
        sessionId: null,
        model: provider === 'codex' ? model || CODEX_MODELS.DEFAULT : model,
        env: runtimeEnv,
        userId: req.user?.id,
      ...
      modelSelection: {
        modelProviderId: provider === 'codex' ? 'openai' : 'anthropic',
        modelId: provider === 'codex' ? model || CODEX_MODELS.DEFAULT : model,
      },
    }, writer);
    ...
    if (createBranch || createPR) {
      try {
        ...
        if (createPR) {
          ...
          prBody += '\n\n---\n*This pull request was automatically created by MedHelp Agent.*';
    ...
    if (stream) {
      ...
      const normalizedAssistantMessages = writer.getNormalizedAssistantMessages();
      ...
      const response = {
        ...
        normalizedMessages: normalizedAssistantMessages,
        tokens: tokenSummary,
        projectPath: finalProjectPath
      };
    ...
    if (cleanup && githubUrl) {
      ...
      setTimeout(() => {
        cleanupProject({
          ...
          allowedExternalRoot: externalProjectCleanupRoot,
        });
      }, 5000);
    }
