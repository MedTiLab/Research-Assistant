...
import { installMcpBundle, McpBundleInstallError } from '../utils/mcpBundleInstaller.js';
...
async function resolveClaudeCommand() {
  return resolveClaudeCodeExecutable({ preferBundledNative: true });
}
...
// POST /api/mcp/bundle/install - Install a self-contained local .mcpb package.
// Uploads are written to a temporary file so large offline bundles do not sit in memory.
export function createMcpBundleInstallHandler({ target = 'claude' } = {}) {
  return (req, res) => installUploadedMcpBundle(req, res, target);
}

async function installUploadedMcpBundle(req, res, target) {
  ...
  try {
    ...
    const upload = multer({
      storage: multer.diskStorage({
        ...
        filename: (_request, _file, callback) => {
          const uniqueName = `medhelp-mcpb-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}.mcpb`;
          callback(null, uniqueName);
        },
      }),
      limits: { files: 1, fileSize: 256 * 1024 * 1024 },
      fileFilter: (_request, file, callback) => {
        const supported = /\.(?:mcpb|dxt)$/i.test(String(file.originalname || ''));
        callback(supported ? null : new Error('Only .mcpb or legacy .dxt packages are supported.'), supported);
      },
    });
    ...
    upload.single('file')(req, res, async (uploadError) => {
      ...
      if (uploadError) {
        return res.status(400).json({ error: 'Failed to receive MCP bundle', details: uploadError.message });
      }
      if (!uploadedPath) {
        return res.status(400).json({ error: 'No MCP bundle was provided.' });
      }

      try {
        const installed = await installMcpBundle({ archivePath: uploadedPath, target });
        return res.status(installed.reused ? 200 : 201).json({
          success: true,
          installed,
          message: installed.reused
            ? `MCP bundle "${installed.displayName}" is already installed and configured.`
            : `MCP bundle "${installed.displayName}" was installed successfully.`,
        });
      } catch (error) {
        const status = error instanceof McpBundleInstallError ? error.status : 500;
        console.error('[mcp] local bundle installation failed:', error);
        return res.status(status).json({
          error: 'Failed to install MCP bundle',
    ...
    if (uploadedPath) await fs.unlink(uploadedPath).catch(() => {});
    console.error('[mcp] multipart setup failed:', error);
    return res.status(500).json({ error: 'Unable to initialize MCP bundle upload', details: error.message });
  }
}
router.post('/bundle/install', createMcpBundleInstallHandler());
...
router.post('/cli/add', async (req, res) => {
  try {
    ...
    const process = spawn(claudeCmd, cliArgs, getSpawnOptions(spawnOptions));
...
router.delete('/cli/remove/:name', async (req, res) => {
  try {
    ...
    const process = spawn(claudeCmd, cliArgs, getSpawnOptions({
      stdio: ['pipe', 'pipe', 'pipe']
    }));
