...
import { PROJECT_MEMORY_RELATIVE_PATH } from '../templates/index.js';
...
const builtInCommands = [
  ...
  {
    name: '/memory',
    description: 'Open the shared project memory file for editing',
    namespace: 'builtin',
    metadata: { type: 'builtin' }
  },
  {
...
// Pi has no equivalents for the legacy UI-only clear/rewind or Claude model/cost/memory handlers.
export function builtInCommandsForProvider(provider) {
  return provider === 'pi' ? builtInCommands.filter(({ name }) => ['/config', '/status'].includes(name)) : builtInCommands;
}
...
`;
    ...
    const total =
      Number(
        tokenUsage.total ??
          ...
          parseInt(process.env.CONTEXT_WINDOW || '256000', 10),
      ) || 256000;
  ...
  '/memory': async (args, context) => {
    ...
    if (!projectPath) {
      return {
        ...
        data: {
          ...
          message: 'Please select a project to access its shared project memory file'
        }
      };
    }
    ...
    // Project initialization normally creates the shared memory file.
    let exists = false;
    try {
    ...
    return {
      ...
      data: {
        ...
        exists,
        message: exists
          ? `Opening shared project memory at ${memoryPath}`
          : `Project memory not found at ${memoryPath}. Refresh the project to initialize it.`
      }
    };
  },
...
router.post('/list', async (req, res) => {
  try {
    ...
    const availableBuiltIns = builtInCommandsForProvider(provider);
    const allCommands = [...availableBuiltIns];
...
router.post('/execute', async (req, res) => {
  try {
    ...
    if (handler) {
      if (!builtInCommandsForProvider(context.provider).some(({ name }) => name === commandName)) {
        return res.status(400).json({ error: 'Unsupported Pi command', message: `${commandName} is not supported in Pi. Conversation and files have not been changed.`, command: commandName });
      }
      try {
