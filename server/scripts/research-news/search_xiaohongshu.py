...
def _redact_cli_args(args: List[str]) -> List[str]:
    """Return CLI args with sensitive values masked for logs."""
    redacted = []
    mask_next = False
    for arg in args:
        if mask_next:
...
def _run_xhs_cli(args: List[str], timeout: int = 120) -> Optional[Dict]:
    ...
    safe_cmd = " ".join(shlex.quote(part) for part in _redact_cli_args(cmd))
    logger.info("[XHS] Running: %s", safe_cmd)
    ...
    for key in ["ALL_PROXY", "all_proxy", "HTTP_PROXY", "http_proxy", "HTTPS_PROXY", "https_proxy"]:
        value = env.get(key, "")
        if isinstance(value, str) and "socks" in value.lower():
            logger.info("[XHS] Ignoring SOCKS proxy env: %s", key)
            env.pop(key, None)
