...
import { encodeProjectPath, getCodexSessions } from './projects.js';
...
function getClaudeUsageSnapshot(entry) {
  ...
  // Keep Claude aligned with Codex by counting model input/output only.
...
export async function getProjectTokenUsageSummary(projectRefs = []) {
  ...
  const projectUsageEntries = await Promise.all(
    normalizedProjectRefs.map(async (projectRef) => {
      const [claudeTotals, codexTotals] = await Promise.all([
      ...
      return [
        projectRef.name,
        mergeUsageTotals(claudeTotals, codexTotals),
      ];
    }),
  );
