...
export const CollapsibleDisplay: React.FC<CollapsibleDisplayProps> = ({
  ...
  return (
    <div
      className={`tool-collapsible-card border border-gray-200 dark:border-gray-800 rounded-lg shadow-sm bg-white dark:bg-gray-900/50 transition-[width,padding,margin] duration-150 ${
        isOpen
          ? 'block w-full px-3 py-2 my-2'
          : 'inline-block w-fit max-w-full px-2.5 py-1 my-1'
      } ${className}`}
    >
      <CollapsibleSection
        title={title}
