...
import { parseToolValue } from '../../../../../../shared/agentToolPresentation.js';
...
export function parseTaskContent(content: unknown): TaskItem[] {
  const value = parseToolValue(content);
  const items = Array.isArray(value) ? value : value?.tasks || (value?.task ? [value.task] : value?.id || value?.task_id ? [value] : null);
  if (Array.isArray(items)) {
    return items.filter((task) => task && (task.id || task.task_id)).map((task) => ({
      id: String(task.id || task.task_id), subject: task.subject || task.title || task.description || 'Task',
      status: task.status || 'pending', owner: task.owner, blockedBy: task.blockedBy || task.blocked_by,
    }));
  }
  ...
  for (const line of lines) {
    ...
    const match = line.match(/#([\w-]+)\.?\s*(?:\[(\w+)\]\s*)?(.+?)(?:\s*\((?:owner:\s*\w+)?\))?$/);
    if (match) {
      ...
      <div className="space-y-px">
        {tasks.map((task) => {
          const config = statusConfig[task.status as keyof typeof statusConfig]
            || (task.status === 'running' ? statusConfig.in_progress : statusConfig.pending);
          return (
            <div
              key={task.id}
            ...
            >
              ...
              <span className={`text-[10px] px-1 py-px rounded border flex-shrink-0 ${['failed', 'interrupted'].includes(task.status) ? 'text-red-600 border-red-200' : config.badgeClass}`}>
