...
export function useFileMentions({ selectedProject, input, setInput, textareaRef }: UseFileMentionsOptions) {
  ...
  useEffect(() => {
    ...
    if (!projectName || !showFileDropdown || loadedProjectName === projectName) {
      return undefined;
    }
    ...
    const fetchProjectFiles = async () => {
      try {
        const response = await api.getFiles(projectName, {
          ...
          maxDepth: FILE_MENTION_MAX_DEPTH,
          showHidden: false,
        });
        if (!response.ok) {
          return;
        ...
        // Ignore aborts from rapid project switches or closing the mention dropdown.
  ...
  }, [loadedProjectName, selectedProject?.name, showFileDropdown]);
