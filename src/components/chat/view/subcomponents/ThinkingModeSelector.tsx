  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <button
        ...
        aria-label={t(`${translationRoot}.buttonTitle`, { mode: currentMode.name })}
        title={t(`${translationRoot}.buttonTitle`, { mode: currentMode.name })}
      >
        <IconComponent className={`${compact ? 'w-3.5 h-3.5' : 'w-5 h-5'} shrink-0 ${currentMode.color}`} />
        <span className="whitespace-nowrap">{currentMode.name}</span>
        <ChevronDown className={`h-3 w-3 shrink-0 text-muted-foreground transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </button>

      {isOpen && (
        <div className={`absolute bottom-full ${compact ? 'left-0' : 'right-0'} z-50 mb-2 ${compact ? 'w-52' : 'w-64'} max-h-[min(400px,70vh)] bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 overflow-y-auto`}>
