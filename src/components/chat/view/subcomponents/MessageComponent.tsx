...
import { piAttachmentReason } from '../../utils/piAttachmentDelivery';
import { Check, Copy, FileImage, FileText, Loader2, RefreshCw, RotateCcw, Share2, User } from 'lucide-react';
...
import { formatDurationSeconds, formatUsageLimitText, getProviderDisplayName } from '../../utils/chatFormatting';
...
const MessageComponent = memo(({ message, index, prevMessage, createDiff, onFileOpen, onShowSettings, onGrantToolPermission, autoExpandTools, showRawParameters, showThinking, hideThinkingFold, selectedProject, provider, onRetry, onRewind, canRewind = false, onShareAssistantMessage, forkControl, turnDurationSeconds }: MessageComponentProps) => {
  ...
  const [isSharingMessage, setIsSharingMessage] = React.useState(false);
  const [shareError, setShareError] = React.useState<string | null>(null);
  const [copyState, setCopyState] = React.useState<'idle' | 'copied' | 'error'>('idle');
  const copyResetTimeoutRef = React.useRef<number | null>(null);
  const projectRoot = getProjectRootPath(selectedProject);
  const displayMessageContent = (value: unknown) => formatProjectRelativePaths(String(value || ''), projectRoot);
  ...
  const assistantCopyText = useMemo(() => {
    ...
    return formatProjectRelativePaths(formatUsageLimitText(message.content), projectRoot).trim();
  }, [message.content, projectRoot]);
  ...
  const canShareAssistantMessage =
    message.type === 'assistant' &&
    !message.isToolUse &&
    !message.isThinking &&
    !message.isStreaming &&
    ...
    Boolean(onShareAssistantMessage);
  const canRetryAnswer =
    message.type === 'error' &&
    Boolean(onRetry) &&
    message.isRetryable !== false;
  const canRewindCurrentTurn =
    canRewind &&
    message.type === 'assistant' &&
    !message.isToolUse &&
    !message.isThinking &&
  ...
  const shareAssistantMessage = async () => {
    if (!onShareAssistantMessage || isSharingMessage) {
      return;
    }
    setIsSharingMessage(true);
    setShareError(null);
    try {
      await onShareAssistantMessage(message);
    } catch (error) {
      setShareError(error instanceof Error ? error.message : t('messageShare.failed'));
    } finally {
  ...
  const copyAssistantMessage = async () => {
    if (!assistantCopyText) {
      return;
    }
    const didCopy = await copyTextToClipboard(assistantCopyText);
  ...
  return (
    <div
      ref={messageRef}
      className={`medical-chat-message chat-message ${message.type} ${isGrouped ? 'grouped' : ''} flex flex-col w-full px-4 sm:px-6`}
    >
      {message.isSkillContent ? (
        /* Collapsible skill content */
        <div className="skill-content-message w-full mb-4">
            ...
            <details className="group">
              <summary className="flex list-none items-center gap-2 px-3 py-2 cursor-pointer text-[13px] select-none hover:bg-purple-50/50 dark:hover:bg-purple-900/20 transition-colors [&::-webkit-details-marker]:hidden">
                ...
                <span className="text-gray-500 dark:text-gray-400 truncate flex-1 font-medium text-xs">
                  {extractSkillContentTitle(displayMessageContent(message.content), t('skill.contentLoaded'))}
                </span>
              </summary>
              <div className="px-4 py-3 border-t border-purple-100 dark:border-purple-800/30 max-h-96 overflow-y-auto">
                <Markdown className="prose prose-sm max-w-none dark:prose-invert" onFileOpen={onFileOpen} projectName={selectedProject?.name} projectRoot={projectRoot}>
        ...
        <div className="flex flex-col items-end w-full mb-4">
          ...
          <div className="bg-blue-600 text-white rounded-2xl rounded-tr-none px-4 py-2.5 shadow-sm max-w-[90%] sm:max-w-[85%]">
            ...
            {provider === 'pi' && message.attachmentDelivery?.some((item) => item.status !== 'sent') && (
              <div role="status" className="mt-2 rounded border border-amber-400/60 p-2 text-xs">
                <strong>以下附件未直接送达模型</strong>
                <ul>{message.attachmentDelivery.filter((item) => item.status !== 'sent').map((item, index) => <li key={index}>{item.name}：{piAttachmentReason(item.reason)}</li>)}</ul>
              </div>
            )}
            {visibleAttachments.length > 0 && (
              <div className="mt-2 flex flex-col gap-2">
                {visibleAttachments.map((attachment, idx) => {
                  ...
                  const displayPath = toProjectRelativeDisplayPath(attachment.path, projectRoot);
                  const attachmentDescription =
                    attachment.kind === 'pdf'
                      ? 'PDF attached to project'
                        ...
                        {attachment.path && (
                          <div className="mt-1 break-all font-mono text-[11px] opacity-70">
                            {onFileOpen ? (
                              <button
                                type="button"
                                onClick={() => onFileOpen(attachment.path || '')}
        ...
        <div className="w-full">
          <div className="flex items-center gap-2 py-0.5">
            ...
            <span className="text-xs text-gray-500 dark:text-gray-400">{displayMessageContent(message.content)}</span>
          </div>
        </div>
      ) : (
        /* Claude/Error/Tool messages */
        <div className={`flex flex-col w-full ${message.isToolUse ? 'mb-1' : 'mb-6'}`}>
          {!isGrouped && (
              ...
              {typeof turnDurationSeconds === 'number' && (
                <>
                  ...
                  <span className="font-mono text-[11px] tabular-nums text-gray-500 dark:text-gray-400">
                    {formatDurationSeconds(turnDurationSeconds)}
                  </span>
                </>
              )}
            </div>
          ...
          <div className="w-full pl-0">
            ...
            {message.isToolUse ? (
              <>
                ...
                {message.toolResult && !shouldHideToolResult(message.toolName || 'UnknownTool', message.toolResult) && (
                  message.toolResult.isError ? (
                    // Keep the error visible, but collapse its verbose output
                    // when automatic tool expansion is disabled.
                    <details
                      id={`tool-result-${message.toolId}`}
                      open={Boolean(autoExpandTools)}
                      className="group/error relative mt-2 rounded border scroll-mt-4 bg-red-50/50 dark:bg-red-950/10 border-red-200/60 dark:border-red-800/40"
                    >
                      <summary className="relative flex cursor-pointer list-none items-center gap-1.5 p-3 text-xs font-medium text-red-700 dark:text-red-300 [&::-webkit-details-marker]:hidden">
                        <svg
                          className="h-3.5 w-3.5 flex-shrink-0 transition-transform group-open/error:rotate-90"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                      ...
                      <div className="relative border-t border-red-200/60 p-3 text-sm text-red-900 dark:border-red-800/40 dark:text-red-100">
                        <Markdown className="prose prose-sm max-w-none prose-red dark:prose-invert" onFileOpen={onFileOpen} projectName={selectedProject?.name} projectRoot={projectRoot}>
              ...
              <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    {(() => {
                      const lines = displayMessageContent(message.content).split('\n').filter((line) => line.trim());
              ...
              <div className="text-sm text-gray-700 dark:text-gray-300">
                {hideThinkingFold ? (
                  <div className="text-gray-600 dark:text-gray-400 text-sm">
                    <Markdown className="prose prose-sm max-w-none dark:prose-invert prose-gray" onFileOpen={onFileOpen} projectName={selectedProject?.name} projectRoot={projectRoot}>
                      {message.content}
                    </Markdown>
                  </div>
                ) : (
              ...
              <div className="text-[15px] text-gray-700 dark:text-gray-300">
                ...
                {(() => {
                  const content = formatProjectRelativePaths(formatUsageLimitText(String(message.content || '')), projectRoot);
                  ...
                  return message.type === 'assistant' ? (
                    <Markdown className="prose prose-md max-w-none dark:prose-invert prose-gray text-[15.5px] leading-relaxed" onFileOpen={onFileOpen} projectName={selectedProject?.name} projectRoot={projectRoot}>
                      {content}
                    </Markdown>
                  ) : (
                ...
                {(canCopyAssistantMessage || canRetryAnswer || canRewindCurrentTurn || canShareAssistantMessage || forkControl) && (
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    {canCopyAssistantMessage && (
                      <button
                        type="button"
                        onClick={() => void copyAssistantMessage()}
                        className="inline-flex items-center gap-1 rounded-md px-1.5 py-1 text-[11px] font-medium text-muted-foreground transition-colors hover:bg-muted/70 hover:text-foreground"
                      >
                        {copyState === 'copied' ? <Check className="h-3 w-3" /> : <Copy className="h-3 w-3" />}
                        {copyState === 'copied'
                          ? t('messageActions.copied')
                          : t('messageActions.copyAnswer')}
                      </button>
                    )}
                    {canRetryAnswer && onRetry && (
                      <button
                        type="button"
                        onClick={() => onRetry()}
                        className="inline-flex items-center gap-1.5 rounded-md border border-red-200/60 bg-red-100 px-3 py-1.5 text-xs font-medium text-red-700 transition-colors hover:bg-red-200 dark:border-red-800/40 dark:bg-red-900/30 dark:text-red-300 dark:hover:bg-red-900/50"
                      >
                        <RefreshCw className="w-3 h-3" />
                        {t('messageActions.retryAnswer')}
                      </button>
                    )}
                    {canRewindCurrentTurn && onRewind && (
                      <button
                        type="button"
                        onClick={onRewind}
                        className="inline-flex items-center gap-1 rounded-md px-1.5 py-1 text-[11px] font-medium text-muted-foreground transition-colors hover:bg-muted/70 hover:text-foreground"
                        title={t('messageActions.rewindDescription')}
                      >
                        <RotateCcw className="h-3 w-3" />
                        {t('messageActions.rewindCurrentChanges')}
                      </button>
                    )}
                    {canShareAssistantMessage && (
                      <button
                        type="button"
                        onClick={() => void shareAssistantMessage()}
                      ...
                      >
                        {isSharingMessage ? <Loader2 className="h-3 w-3 animate-spin" /> : <Share2 className="h-3 w-3" />}
                        {isSharingMessage ? t('messageShare.creating') : t('messageShare.button')}
                      </button>
                    )}
                    {forkControl}
                    {copyState === 'error' ? (
                      <span className="text-[11px] text-red-500 dark:text-red-400">
                        {t('messageActions.copyFailed')}
                      </span>
                    ) : null}
                    {shareError ? (
                      <span className="text-[11px] text-red-500 dark:text-red-400">
                    ...
                    {message.type === 'error' && message.errorType === 'usage_limit' && (
