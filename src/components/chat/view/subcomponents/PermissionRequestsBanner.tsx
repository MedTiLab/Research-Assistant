...
import { AskUserQuestionPanel, WorkbenchPermissionPanel } from '../../tools/components/InteractiveRenderers';
import PlanApprovalPanel from '../../tools/components/InteractiveRenderers/PlanApprovalPanel';

registerPermissionPanel('AskUserQuestion', AskUserQuestionPanel);
registerPermissionPanel('ExitPlanMode', PlanApprovalPanel);
[
  ...
  'calendar_create', 'calendar_update', 'thesis_create', 'thesis_update',
  'thesis_chapter_add', 'thesis_chapter_update', 'thesis_milestone_add', 'thesis_milestone_update', 'thesis_log_add',
  'submission_create', 'submission_update', 'daily_review_save', 'attendance_start', 'attendance_end',
  'focus_log', 'habit_create', 'habit_entry_update',
].forEach((name) => registerPermissionPanel(`mcp__medhelp_workbench__${name}`, WorkbenchPermissionPanel));
  ...
  return (
    <div className="mb-3 space-y-2">
      {pendingPermissionRequests.map((request) => {
        const requestProvider = request.runtimeId || request.provider || provider;
        const isPi = requestProvider === 'pi';
        const canRemember = requestProvider === 'claude' || (isPi && !['AutomationCreate', 'AutomationUpdate', 'McpAuthorize'].includes(request.toolName));
        ...
        const permissionEntry = isPi ? `${request.toolName} (exact input, this project)` : buildToolPermissionEntry(request.toolName, rawInput);
        const settings = getProviderSettings(requestProvider);
        const alreadyAllowed = !isPi && permissionEntry ? settings.allowedTools.includes(permissionEntry) : false;
        const rememberLabel = alreadyAllowed ? 'Allow (saved)' : 'Allow & remember';
        const matchingRequestIds = isPi ? [request.requestId] : permissionEntry
          ? pendingPermissionRequests
              .filter(
                (item) =>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {canRemember && (
                <button
                  type="button"
                  onClick={() => {
                    if (permissionEntry && !alreadyAllowed && !isPi) {
