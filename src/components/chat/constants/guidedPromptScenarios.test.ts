...
describe('CHAT_QUICK_ACTION_SCENARIOS', () => {
  ...
  it('binds only shortcuts with a relevant installed-skill workflow', () => {
    ...
    expect(CHAT_QUICK_ACTION_SCENARIOS.find((scenario) => scenario.id === 'watch-news')?.skills)
      .toEqual(['agent-reach']);
    expect(CHAT_QUICK_ACTION_SCENARIOS.find((scenario) => scenario.id === 'create-automation')?.skills)
      .toEqual([]);
  });
