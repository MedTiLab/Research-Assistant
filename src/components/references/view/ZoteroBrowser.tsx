...
interface ZoteroBrowserProps {
  projectName?: string;
  onClose: () => void;
  onImportComplete: (result?: { synced?: number; linked?: number; total?: number; ids?: string[] }) => void;
}
    ...
    return (
      <div className="space-y-4">
        ...
        {loadingCollections ? (
          ...
          <div className="py-4 text-center text-sm text-foreground">{error}</div>
        ) : (
  ...
  if (step === 'items') {
    return (
      <div className="space-y-3">
        ...
        {error && <div className="text-sm text-foreground">{error}</div>}
