      ...
      return {
        ...
        content: typeof todo.content === 'string' && todo.content.trim()
          ...
          : typeof todo.text === 'string' && todo.text.trim()
            ? todo.text
            : `Todo ${index + 1}`,
        status: typeof todo.status === 'string' && todo.status.trim() ? todo.status : 'pending',
        priority: typeof todo.priority === 'string' && todo.priority.trim() ? todo.priority.trim() : null,
      };
    }

    return {
      id: `todo-${index}`,
      content: typeof todo === 'string' && todo.trim() ? todo : `Todo ${index + 1}`,
      status: 'pending',
      priority: null,
    };
  });
