...
const Onboarding = ({ onComplete }) => {
  ...
  }, [activeLoginProvider, localKernelHttpBaseUrl, localKernelSessionToken]);
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3 rounded-lg border border-border bg-card p-4">
              <label className="flex items-start gap-3 text-sm text-foreground">
                ...
                />
                <span>
                  Allow my usage data to improve MedHelp models and features (recommended). You can still continue without this and change it anytime in Settings.
                </span>
              </label>
            </div>
          </div>
            ...
            <div className="space-y-3">
              ...
              <div className={`border rounded-lg p-4 transition-colors ${
                codexAuthStatus.authenticated
                  ...
                  : codexAuthStatus.cliAvailable === false
                    ? 'bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-800'
                  : 'border-border bg-card'
              }`}>
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-10 h-10 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center shrink-0">
                      <CodexLogo className="w-6 h-6" />
                    </div>
                    <div className="min-w-0">
                      <div className="font-medium text-foreground flex items-center gap-2">
                        Codex CLI
                        {codexAuthStatus.authenticated && <Check className="w-4 h-4 text-green-500 shrink-0" />}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        {codexAuthStatus.loading ? 'Checking...' :
                         codexAuthStatus.cliAvailable === false ? 'Install Codex CLI first' :
                         codexAuthStatus.authenticated ? codexAuthStatus.email || 'Connected' : 'Not connected'}
                      </div>
                      {codexAuthStatus.cliAvailable === false && codexAuthStatus.installHint && (
                        <p className="text-xs text-amber-800 dark:text-amber-200 mt-1">{codexAuthStatus.installHint}</p>
                      )}
                    </div>
                  </div>
                  {!codexAuthStatus.authenticated && !codexAuthStatus.loading && codexAuthStatus.cliAvailable !== false && (
                    <button
