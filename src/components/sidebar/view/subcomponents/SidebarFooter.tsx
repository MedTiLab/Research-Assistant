import { Settings, ArrowUpCircle, CircleHelp, Github } from 'lucide-react';
...
const OPEN_SOURCE_URL = 'https://github.com/MedTiLab/Research-Assistant';
  ...
  return (
    <div className="relative z-40 flex-shrink-0 overflow-visible" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0)' }}>
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="hidden h-7 min-w-0 items-center gap-1.5 px-4 text-xs text-muted-foreground transition-colors hover:text-foreground md:flex"
        aria-label={t('actions.openSource')}
        title={OPEN_SOURCE_URL}
      >
        <Github className="h-3.5 w-3.5 shrink-0" />
        <span className="truncate">{t('actions.openSource')}</span>
      </a>
      ...
      <div className="p-3 pb-20 md:hidden">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-2 flex h-10 min-w-0 items-center gap-2 rounded-xl px-3 text-sm text-muted-foreground transition-colors hover:bg-muted/40 hover:text-foreground"
          aria-label={t('actions.openSource')}
          title={OPEN_SOURCE_URL}
        >
