...
// MCP servers with user/project scopes.
function ScopedMcpServers({
  agent = 'claude',
  servers,
  onAdd,
  onEdit,
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        {t(`mcpServers.description.${agent}`)}
      </p>
      ...
      <div className="space-y-2">
        {servers.map(server => (
          <div key={`${server.scope}:${server.projectPath || ''}:${server.id}`} className="bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-center gap-2 ml-4">
                ...
                <Button
                  onClick={() => onDelete(server.id, server.scope, server.projectPath || '')}
                  variant="ghost"
                  size="sm"
...
export default function McpServersContent({ agent, ...props }) {
  if (agent === 'claude') {
    return <ScopedMcpServers agent={agent} {...props} />;
  }
  if (agent === 'codex') {
