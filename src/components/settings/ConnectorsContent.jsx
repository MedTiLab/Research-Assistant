...
import { ArrowLeft, Database, Mail, Plug } from 'lucide-react';
import { useTranslation } from 'react-i18next';

import { Badge } from '../ui/badge';
import { Button } from '../ui/button';
import { authenticatedFetch, api } from '../../utils/api';
import DatabaseApiAccessContent from './DatabaseApiAccessContent';
...
export default function ConnectorsContent() {
  ...
  const loadStatus = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      const [profileRes, senderRes, databaseRes] = await Promise.all([
        authenticatedFetch('/api/user/profile'),
        api.settings.autoResearchEmail(),
        authenticatedFetch('/api/settings/database-api-access', { forceCloud: true }),
      ]);
      ...
      const databaseConfigured = databaseRes.ok ? Boolean(databaseData?.connection?.connected) : false;
  ...
  const connectors = [
    ...
    },
    {
      id: 'database',
      icon: Database,
      title: t('connectors.database.title'),
      description: t('connectors.database.description'),
  ...
  if (activeConnector && activeMeta) {
    ...
    return (
      <div className="space-y-4">
        ...
        {activeConnector === 'database' && <DatabaseApiAccessContent embedded onStatusChange={loadStatus} />}
      </div>
    );
  }
