  ...
  return (
    <>
      ...
      <footer className={`border-t ${dark ? 'border-white/[0.06] bg-[#0d0f13]' : 'border-gray-100 bg-gray-50'}`}>
        ...
        <div className={`border-t px-6 py-5 ${dark ? 'border-white/[0.04]' : 'border-gray-200/60'}`}>
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
            <p className={`whitespace-nowrap text-xs ${dark ? 'text-white/25' : 'text-gray-400'}`}>{t('footer.copyright')}</p>
            <a
              href="https://beian.miit.gov.cn/"
              target="_blank"
              rel="noreferrer"
              className={`whitespace-nowrap text-xs transition-colors ${dark ? 'text-white/25 hover:text-white/50' : 'text-gray-400 hover:text-gray-600'}`}
            >
              {t('footer.icp')}
            </a>
          </div>
