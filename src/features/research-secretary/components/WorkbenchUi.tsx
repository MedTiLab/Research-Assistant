...
export function WorkbenchPage({
  ...
  return (
    <div className={cn('h-full min-h-0 overflow-y-auto bg-muted/25', fill && 'xl:overflow-hidden')}>
      ...
      >
        {onMenuClick && (
          <button type="button" onClick={onMenuClick} className="mb-4 inline-flex h-9 w-9 items-center justify-center rounded-lg border border-border bg-card text-muted-foreground shadow-sm sm:hidden" aria-label="打开导航">
...
export function formatWorkbenchDate(value?: string, options?: Intl.DateTimeFormatOptions) {
  if (!value) return '—';
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return '—';
  return new Intl.DateTimeFormat(undefined, options ?? { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }).format(date);
}

export function getProjectLabel(projectId: string | undefined, projectNames: Map<string, string>) {
  if (!projectId) return '跨项目';
