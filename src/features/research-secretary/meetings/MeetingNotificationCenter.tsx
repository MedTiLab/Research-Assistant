...
export default function MeetingNotificationCenter({
  ...
  return (
    <div ref={rootRef} className="relative flex shrink-0 items-center justify-center">
      {variant === 'rail' ? (
        <Tooltip content={t('notifications.title')} position="right" delay={120}>
          {trigger}
        </Tooltip>
      ) : (
        trigger
                ...
                <div key={item.id} className="border-b border-border/50 px-4 py-3 last:border-b-0">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="shrink-0 text-[10px] text-muted-foreground">
                      {item.deliveredAt
                        ? new Date(item.deliveredAt).toLocaleString(locale, { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })
                        : ''}
                    </div>
                  </div>
