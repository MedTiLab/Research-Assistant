...
export default function ActionItemBoard({ actions, api, onChanged }: { actions: MeetingActionItem[]; api: ResearchSecretaryApi; onChanged: () => Promise<unknown> }) {
  ...
  const run = async (id: string, operation: () => Promise<unknown>) => {
    ...
    catch (nextError) { setError(nextError instanceof Error ? nextError.message : '行动项操作失败'); }
    finally { setBusyId(null); }
  };
  if (!actions.length) return <EmptyState>尚未确认行动项。纪要中的候选事项需要你手动点击“转行动项”。</EmptyState>;
  return <div className="space-y-2">
    {actions.map((action) => <div key={action.id} className="flex items-start gap-3 rounded-lg border border-border/60 p-3">
      ...
      <div className="min-w-0 flex-1"><div className={cn('text-sm text-foreground', action.status === 'done' && 'line-through opacity-60')}>{action.content}</div>{action.dueDate && <div className="mt-1 text-xs text-muted-foreground">截止：{action.dueDate}</div>}<div className="mt-2">{action.taskId ? <StatusBadge tone="success">已转任务 #{action.taskId}</StatusBadge> : action.projectId ? <Button size="sm" variant="ghost" disabled={busyId === action.id} onClick={() => void run(action.id, () => api.promoteActionToTask(action.id))}>{busyId === action.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <ListTodo className="h-3.5 w-3.5" />}转正式任务</Button> : <span className="text-[11px] text-muted-foreground">关联项目后可转正式任务</span>}</div></div>
      <StatusBadge tone={action.status === 'done' ? 'success' : 'neutral'}>{action.status === 'done' ? '已完成' : '待处理'}</StatusBadge>
    </div>)}
