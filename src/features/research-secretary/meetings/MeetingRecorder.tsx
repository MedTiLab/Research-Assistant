...
export default function MeetingRecorder({
  ...
  const beginSegment = () => {
    ...
    recorder.addEventListener('error', () => {
      setError(`录音分片 ${segmentIndex + 1} 采集失败`);
    });
    recorder.addEventListener('stop', () => {
      ...
      void api.uploadRecordingChunk(meetingId, {
        ...
        setError(uploadError instanceof Error ? uploadError.message : '录音分片上传失败');
      }).finally(resolveJob);
    });
    recorder.start();
  ...
  const start = async () => {
    if (!navigator.mediaDevices?.getUserMedia || typeof MediaRecorder === 'undefined') {
      setError('当前环境不支持浏览器录音。');
      return;
    }
    let consented = window.localStorage.getItem(CONSENT_KEY) === 'accepted';
    if (!consented) {
      consented = window.confirm('组会可能包含未发表数据。继续后，录音分片会上传至 OpenAI 进行转写。是否同意？');
      if (!consented) return;
    ...
    try {
      ...
      setError(startError instanceof Error ? startError.message : '无法开始录音');
      updateState('idle');
    }
  };
  ...
  return <div className="rounded-lg border border-border/60 bg-muted/20 p-3">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div><div className="text-sm font-medium text-foreground">录音与中文转写</div><div className="mt-1 text-xs text-muted-foreground">30 秒独立分片，保留 5 秒重叠并自动去除重复转写。</div></div>
      <div className="flex gap-2">
        {state === 'idle' ? <Button size="sm" onClick={() => void start()}><Mic className="h-4 w-4" />开始录音</Button> : <>
          <Button size="sm" variant="outline" disabled={state === 'starting' || state === 'stopping'} onClick={togglePause}>{state === 'paused' ? <CirclePlay className="h-4 w-4" /> : <CirclePause className="h-4 w-4" />}{state === 'paused' ? '继续' : '暂停'}</Button>
          <Button size="sm" variant="destructive" disabled={state === 'starting' || state === 'stopping'} onClick={() => void finalizeRecording()}>{state === 'stopping' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Square className="h-4 w-4" />}停止</Button>
        </>}
      </div>
    </div>
