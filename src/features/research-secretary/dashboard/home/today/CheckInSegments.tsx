  ...
  if (ordered.length === 0) {
    return (
      <EmptyHint
        ...
        title="今天还没有打卡记录"
        description="点击「开始工作」记录第一段，结束后会显示时长。"
        className="py-5"
      />
    );
  }
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block text-sm font-medium tabular-nums text-foreground">
                ...
                {log.endedAt ? formatTime(log.endedAt) : <span className="text-primary">进行中</span>}
              </span>
              <span className="mt-0.5 block text-[11px] text-muted-foreground">
                {log.open ? `已工作 ${formatDuration(minutes)}` : formatDuration(minutes)}
              </span>
            </span>
            {log.open && (
