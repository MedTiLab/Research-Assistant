  ...
  if (ordered.length === 0) {
    return (
      <EmptyHint
        ...
        title={t('focusSessions.emptyTitle')}
        description={t('focusSessions.emptyDescription')}
        className="py-5"
      />
    );
  }
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block truncate text-sm font-medium text-foreground">
              {session.taskTitle?.trim() || t('pomodoro.unnamed')}
            </span>
            <span className="mt-0.5 block text-[11px] tabular-nums text-muted-foreground">
              {formatTime(session.createdAt, locale)}
              <span className="mx-1.5 text-border">·</span>
              {formatDuration(session.minutes, t)}
            </span>
          </span>
          {onDelete && (
            <button
              type="button"
              disabled={busy}
              aria-label={t('focusSessions.deleteAria', { index: index + 1 })}
