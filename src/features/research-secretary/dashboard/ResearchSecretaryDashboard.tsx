...
export default function ResearchSecretaryDashboard({
  ...
  const refreshTodayStatus = useCallback(() => {
    void Promise.all([
      ...
      setStateError(cause instanceof Error ? cause.message : t('home.errors.loadToday'));
    });
  }, []);
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      setStateError(null);
    }).catch((cause) => setStateError(cause instanceof Error ? cause.message : t('home.errors.refreshPlan')));
  }, [latestMessage]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) setStateError(cause instanceof Error ? cause.message : t('home.errors.loadPlan'));
      }
    };
    void load();
  ...
  const savePlan = useCallback(() => {
    ...
    }).catch((cause) => setStateError(cause instanceof Error ? cause.message : t('home.errors.savePlan')));
  }, [focus, goal]);
  ...
  const saveInboxNote = useCallback(() => {
    ...
    void workbenchStateApi.saveNote({ kind: 'inbox', content: text }).then((note) => {
      ...
      setInboxDraft('');
      setStateError(null);
    }).catch((cause) => setStateError(cause instanceof Error ? cause.message : t('home.errors.saveInbox')));
  }, [inboxDraft]);
  ...
  const deleteInboxNote = useCallback((id: string) => {
    void workbenchStateApi.deleteNote(id).then(() => {
      ...
      setStateError(null);
    }).catch((cause) => setStateError(cause instanceof Error ? cause.message : t('home.errors.deleteInbox')));
  }, []);
  ...
  const addCalendarTodo = useCallback((date: string, title: string) => {
    void workbenchStateApi.createCalendarTodo({ title, date }).then((todo) => {
      ...
      setStateError(null);
    }).catch((cause) => setStateError(cause instanceof Error ? cause.message : t('home.errors.addTodo')));
  }, []);
  ...
  const toggleCalendarTodo = useCallback(async (id: string) => {
    ...
    try {
      ...
      setStateError(cause instanceof Error ? cause.message : t('home.errors.updateTodo'));
    }
  }, [calendarTodos]);
  ...
  const toggleResearchTask = useCallback(async (task: ResearchTask, done: boolean) => {
    try {
      ...
      setStateError(cause instanceof Error ? cause.message : t('home.errors.updateTask'));
    }
  }, [api, refreshSnapshot]);
  ...
  const deleteCalendarTodo = useCallback((id: string) => {
    void workbenchStateApi.deleteCalendarTodo(id).then(() => {
      ...
      setStateError(null);
    }).catch((cause) => setStateError(cause instanceof Error ? cause.message : t('home.errors.deleteTodo')));
  }, []);
  ...
  const signals = useMemo<HomeSignal[]>(() => {
    ...
    if (overdueTasks.length > 0) {
      items.push({
        ...
        tone: 'danger',
        title: t('home.signals.overdue', { count: overdueTasks.length }),
    ...
    if (todayStatus && !todayStatus.reviewCompleted) {
      items.push({
        ...
        tone: 'primary',
        title: t('home.signals.noReview'),
        meta: todayStatus.habitTotal > 0
          ? t('home.signals.habitProgress', { completed: todayStatus.habitCompleted, total: todayStatus.habitTotal })
          : t('home.signals.recordConclusion'),
      ...
      items.push({
        ...
        icon: <ClipboardCheck className="h-3.5 w-3.5" />,
        tone: 'evidence',
        title: t('home.signals.habitsRemaining', { count: todayStatus.habitTotal - todayStatus.habitCompleted }),
        meta: t('home.signals.habitsDone', { completed: todayStatus.habitCompleted, total: todayStatus.habitTotal }),
        onClick: () => onNavigate('dailyReview'),
      });
    }
    ...
    snapshot.submissions
      ...
      .forEach((submission) => {
        items.push({
          ...
          title: `${submission.journal} · ${t(`submissionStatus.${submission.status}`)}`,
          meta: submission.nextAction
            || (submission.deadline ? t('common.deadline', { date: new Intl.DateTimeFormat(locale, { month: 'numeric', day: 'numeric' }).format(new Date(submission.deadline)) }) : t('home.signals.viewSubmission')),
    ...
    snapshot.advisorActions
      ...
      .forEach((action) => {
        items.push({
          ...
          title: action.title,
          meta: action.nextAction || t('home.signals.advisorAssigned', {
            name: action.advisorName || t('common.advisorFallback'),
            status: action.status === 'in_progress' ? t('status.inProgress') : t('status.pending'),
          }),
    ...
    activeRuns
      ...
      .forEach((run) => {
        items.push({
          ...
          meta: run.nextAction || t('home.signals.waitingConfirm'),
    ...
    snapshot.automationJobs
      ...
      .forEach((job) => {
        items.push({
          ...
          tone: 'danger',
          title: t('home.signals.jobFailed', { name: job.name }),
          meta: job.description || t('home.signals.checkAutomation'),
          onClick: () => onNavigate('automation'),
        });
      });
    ...
    if (unreadLiterature.length > 0) {
      items.push({
        ...
        tone: 'evidence',
        title: t('home.signals.unreadPapers', { count: unreadLiterature.length }),
    ...
    if (runningAgentCount > 0) {
      items.push({
        ...
        icon: <Bot className="h-3.5 w-3.5" />,
        tone: 'primary',
        title: t('home.agentsRunning', { count: runningAgentCount }),
        meta: t('home.signals.viewAgents'),
        onClick: () => onNavigate('automation'),
      });
    }
    ...
    activeRuns, locale, onNavigate, overdueTasks, runningAgentCount, snapshot.advisorActions,
    snapshot.automationJobs, snapshot.submissions, t, todayStatus, unreadLiterature,
  ]);
  ...
  const quickLinks = useMemo<QuickLink[]>(() => ([
    ...
    { key: 'chat', label: t('home.quickLinkChat'), icon: <Target className="h-3.5 w-3.5" />, tab: 'chat' as AppTab },
  ])
    .filter((link) => isAppTabVisible(link.tab))
    .map(({ tab, ...link }) => ({ ...link, onClick: () => onNavigate(tab) })), [onNavigate, t]);
    ...
    <WorkbenchPage fill onMenuClick={onMenuClick}>
      ...
      <div className="flex min-h-0 flex-col gap-3 pb-6 xl:h-full xl:pb-0">
        <HomeHero
          ...
          onOpenChat={() => (projects[0] ? onNavigate('chat') : startAssistant(t('home.chatPrompt')))}
        />
          ...
          <div className="flex min-w-0 flex-col gap-3 xl:h-full xl:min-h-0 xl:overflow-y-auto xl:overscroll-contain xl:[scrollbar-gutter:stable]">
            <div className="flex-none">
              <NextMeetingCard
                ...
                onPrepare={() => startAssistant(
                  nextMeeting ? t('home.prepareMeeting', { title: nextMeeting.title }) : t('home.prepareNextMeeting'),
            ...
            <Panel className="xl:flex-[1_0_auto]">
              <PanelHead
                ...
                title={t('home.inboxTitle')}
                hint={t('home.inboxHint')}
                action={<span className="flex-shrink-0 rounded-full bg-muted px-2.5 py-1 text-[11px] text-muted-foreground">{t('home.inboxCount', { count: inboxNotes.length })}</span>}
              />
              <HomeInbox
                draft={inboxDraft}
                notes={inboxNotes}
